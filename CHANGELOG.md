...
- **A stale not-afk event is no longer read as "you are at the keyboard".** The check for whether you are currently away only asked what the newest event's status was, so as long as the AFK feed's last word was a not-afk event the watcher believed you were sitting there — for 20 hours, in the case that prompted this. Nothing was asked about, and the live "still AFK" dialog never appeared either. A not-afk event now only means presence while it is still growing (5 minutes' grace, well above the ~2 minutes a live feed can go quiet between reports). Coming back to a machine whose AFK feed died therefore gets you the ongoing-period prompt, and users of a feed that emits few explicit afk events (aw-watcher-window-wayland) get the live "still AFK" dialog for the first time.
