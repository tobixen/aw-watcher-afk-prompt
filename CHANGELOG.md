...
- **`kill -USR1 <pid>` dumps every thread's stack to the journal** without stopping the watcher, for finding out where it is stuck when it stops working without saying why. The live "still AFK" dialog is now logged when it is shown, like the other dialogs' outcomes are.
...
- **Cancelling the split dialog no longer leaves a frozen window on screen.** The window stayed there, unresponsive to clicks and to the window manager, until some later prompt happened to clear it away; pressing OK was never affected.
